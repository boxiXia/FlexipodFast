#ifndef SIMPLE_CPP_SOCKETS
#define SIMPLE_CPP_SOCKETS
// modified from https://github.com/computersarecool/cpp_sockets
...
public:
    ...
    ssize_t send_message(const char* buffer, int len, int flags = 0);
};
...
ssize_t UDPClient::send_message(const char* buffer, int len, int flags = 0)
{
    return sendto(m_socket, buffer, len, flags, reinterpret_cast<sockaddr*>(&m_addr), sizeof(m_addr));
};
